import Link from "next/link";
import { ChevronRight, Flame, Layers, Sparkles } from "lucide-react";

import { formatRelativeTime } from "@/lib/format-date";
import { cn } from "@/lib/utils";

import type { FlashcardDeckListItem } from "../types";

interface DeckListProps {
  decks: FlashcardDeckListItem[];
}

export function DeckList({ decks }: DeckListProps) {
  return (
    <ul className="flex flex-col gap-2.5">
      {decks.map((d) => (
        <li key={d.id}>
          <Link
            href={`/app/flashcards/${d.id}`}
            className="flex items-center gap-3.5 rounded-lg border border-border bg-card p-4 transition-colors hover:border-primary/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
          >
            <span
              aria-hidden
              className={cn(
                "flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-md",
                d.dueCards > 0
                  ? "bg-warning/12 text-warning"
                  : d.newCards > 0
                    ? "bg-accent text-primary"
                    : "bg-success/12 text-success",
              )}
            >
              {d.dueCards > 0 ? (
                <Flame className="h-[18px] w-[18px]" strokeWidth={2} />
              ) : d.newCards > 0 ? (
                <Sparkles className="h-[18px] w-[18px]" strokeWidth={2} />
              ) : (
                <Layers className="h-[18px] w-[18px]" strokeWidth={2} />
              )}
            </span>
            <div className="flex min-w-0 flex-1 flex-col">
              <div className="flex items-center gap-2">
                <span className="text-[11px] font-bold uppercase tracking-wider text-primary">
                  {d.subjectName}
                </span>
                <span className="text-[11px] font-semibold text-muted-foreground/70">
                  {formatRelativeTime(d.updatedAt)}
                </span>
              </div>
              <div className="mt-0.5 truncate text-[15px] font-bold tracking-tight">
                {d.title}
              </div>
              <div className="mt-0.5 text-[13px] text-muted-foreground">
                {formatDeckStats(d)}
              </div>
            </div>
            <ChevronRight
              className="h-[18px] w-[18px] flex-shrink-0 text-muted-foreground/70"
              strokeWidth={2}
              aria-hidden
            />
          </Link>
        </li>
      ))}
    </ul>
  );
}

function formatDeckStats(d: FlashcardDeckListItem): string {
  const parts: string[] = [`${d.totalCards} cards`];
  if (d.dueCards > 0) parts.push(`${d.dueCards} due`);
  if (d.newCards > 0) parts.push(`${d.newCards} new`);
  return parts.join(" · ");
}
